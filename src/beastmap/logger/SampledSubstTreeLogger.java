...
public class SampledSubstTreeLogger extends BEASTObject implements Loggable {
    ...
    BranchSubstLogger branchLengths;
    ...
    public void initAndValidate() {
        ...
        if (parameterInput.get().size() == 0 && clockModelInput.get() == null && samplerInput.get().size() == 0) {
        	someMetaDataNeedsLogging = false;
        	return;
        }
    	someMetaDataNeedsLogging = true;
        branchLengths = lengthsInput.get();
    ...
    public void log(long sampleNr, PrintStream out) {
        ...
        List<StochasticMapProperty> samplers = samplerInput.get();
        for (StochasticMapProperty sampler : samplers) {
        	sampler.sampleMutations(sampleNr);
        	metadata.add((Function)sampler);
        }
        

        if (branchLengths != null) {
        	branchLengths.sampleMutations(sampleNr);
        }
    ...
    String toNewick(Node node, List<Function> metadataList, BranchRateModel.Base branchRateModel) {
				...
				for (Function metadata : metadataList) {
					if (metadata instanceof Parameter<?>) {
						...
						if (p.getMinorDimension2() > node.getNr()) {
							...
							String id = ((BEASTObject) metadata).getID();
							if (metadata instanceof StochasticMapProperty) {
								id = ((StochasticMapProperty)metadata).getName();
							}
							buf2.append(id);
							buf2.append('=');
							if (dim > 1) {
								...
								} else if (metadata instanceof StochasticMapProperty) {
									StochasticMapProperty rp = (StochasticMapProperty) metadata;
									append(buf2, rp.getPropertyOfNode(node));
								} else {
        ...
        buf.append(":");
        if (branchLengths != null) {
        	double val = node.isRoot() ? 0 : branchLengths.getArrayValue(node.getNr());
            appendDouble(buf, val);
        } else {
