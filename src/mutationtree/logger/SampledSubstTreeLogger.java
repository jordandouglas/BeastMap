...
public class SampledSubstTreeLogger extends BEASTObject implements Loggable {
	...
	final public Input<List<BranchSubstLogger>> samplerInput = new Input<>("sampler", "each sampler's metric of interest will be logged onto the branches",new ArrayList<>());
    ...
    public void log(long sampleNr, PrintStream out) {
        ...
        List<BranchSubstLogger> samplers = samplerInput.get();
        for (BranchSubstLogger sampler : samplers) {
        	sampler.sampleMutations(sampleNr);
        	metadata.add(sampler);
        }
				...
				for (Function metadata : metadataList) {
					if (metadata instanceof Parameter<?>) {
						...
						if (p.getMinorDimension2() > node.getNr()) {
							...
							if (metadata instanceof BranchSubstLogger) {
								id = ((BranchSubstLogger)metadata).getName();
							}
							buf2.append(id);
							buf2.append('=');
							if (dim > 1) {
