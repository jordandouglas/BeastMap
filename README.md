...
These loggers can be further modified in the `MCMC` tab, e.g. to modify the logging frequency and output file names.

The first `burnin` steps of the MCMC chain will not have any stochastic mapping. This is because the initial states of the chain often present numerical issues due to long branches and high rates. 
